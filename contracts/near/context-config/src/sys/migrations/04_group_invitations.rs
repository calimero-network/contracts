...
/// Pre-migration group meta without invitation fields.
#[derive(Debug)]
#[near(serializers = [borsh])]
...
pub fn migrate() {
    ...
    // The new `invitation_commitments` and `used_invitations` fields are
    // initialized as empty collections keyed by their group-specific prefix.
    //
    // Because NEAR SDK `IterableMap`/`IterableSet` are prefix-keyed, the new
    // empty collections don't conflict with existing storage.
    env::state_write(&state);

    for (group_id, _group) in state.groups.iter() {
        env::log_str(&format!(
            "Migrated group `{}` (added invitation_commitments, used_invitations)",
            Repr::new(*group_id)
        ));
    }
}
